WITH customer_category_preferences AS (
    SELECT
        ...
        -- Determine if the customer has ever purchased a children's bicycle
        MAX(CASE WHEN product_category = 'Children Bicycles' THEN 1 ELSE 0 END) AS has_kids -- Moved has_kids here
    FROM
...
customer_segments AS (
    SELECT
        ...
        zip_code,
        full_address,
        has_kids, -- Include has_kids from customer_category_preferences
...
final_result AS (
    SELECT
        ...
        cs.has_kids, -- has_kids is now directly from customer_segments
        sd.store_name,
    ...
    JOIN
        customer_segments cs ON sd.customer_id = cs.customer_id -- Join directly with customer_segments
    LEFT JOIN
    ...
    GROUP BY
        ...
        cs.city,
        cs.full_address,
        cs.has_kids, -- Group by has_kids from customer_segments
        sd.store_name,
        cs.customer_segment,
        cf.favorite_brand,
        cf.favorite_product
